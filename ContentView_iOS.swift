this is a new install//2025 SEP 13 2151 ContentView_iOS
...
class JournalManager: ObservableObject {
    ...
    private let migrationFlagKey = "didMigrateLocalToICloudJournal_iOS_altfile"
    ...
    func saveEntries() {
        do {
            ...
            print("Saved to: \(currentStoreURL.path)")
        } catch {
    ...
    func loadEntries() {
        do {
            let url = currentStoreURL
            // Ensure directory exists to avoid errors when writing later
            try? ensureCurrentDirectoryExists()
            // If the file doesn't exist yet, start with empty entries
            ...
            print("Loaded \(entries.count) entries from: \(url.path)")
        } catch {
    ...
    func getStorageInfo() -> String {
        ...
        let location = (ubiquityContainerURL != nil) ? "iCloud Drive" : "Local Documents"
        return """
    ...
    private func migrateLocalFileToICloudIfNeeded() {
        ...
        do {
            ...
            if FileManager.default.fileExists(atPath: iCloudURL.path) == false {
                ...
                print("Migrated local journal to iCloud Drive (alt iOS file)")
            }
            UserDefaults.standard.set(true, forKey: migrationFlagKey)
        } catch {
    ...
    // MARK: - iCloud metadata observations
                ...
                Button("Take Selfie") {
                    ensureCameraAvailableAndAuthorized { ok in
                        if ok {
                            ...
                            // Optionally, you can present an alert here in the future
                        }
                    }
                }
...
struct DocumentScannerView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIViewController {
        if VNDocumentCameraViewController.isSupported {
            ...
            // Fallback: immediately dismiss and return a harmless controller
